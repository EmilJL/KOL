...
class NewUser extends Component{
  ...
  handleCreateUser = () => {
    ...
    if (allValid) {
      return this.props.createUser(this.state.email, this.state.password);
    }
    else{
      return null;
    }
  ...
  handleFocusLoss = (type) => {
    switch(type){
        case 'name':
              if (this.state.name === '') {
                this.setState({inputStyleName: S.inputNotInFocus, inputFontStyleName: S.inputFontNotInFocus, errorTextName: 'Indtast venligst et navn'});
              }
              else{
